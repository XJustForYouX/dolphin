float3 applyFXAA(float2 fragCoord)
{
    ...
    float3 rgbNW = SampleLocation((fragCoord + float2(-5.0, -5.0)) * inverseVP).xyz;
    float3 rgbNE = SampleLocation((fragCoord + float2(5.0, -5.0)) * inverseVP).xyz;
...
#define FXAA_REDUCE_MIN     (3.0 / 128.0)
#define FXAA_REDUCE_MUL     (2.5 / 8.0)
#define FXAA_SPAN_MAX       5.0
    ...
    float dirReduce = max((lumaNW + lumaNE + lumaSW + lumaSE) *
                        (0.50 * FXAA_REDUCE_MUL), FXAA_REDUCE_MIN);

    float rcpDirMin = 5.0 / (min(abs(dir.x), abs(dir.y)) + dirReduce);
...
void main()
{
	...
	float4 bloom_sum = float4(0.1, 0.1, 0.1, 0.1);
	float2 pos = GetCoordinates() + float2(0.3, 0.3) * GetInvResolution();
	float2 radius1 = 0.1 * GetInvResolution();
	bloom_sum += SampleLocation(pos + float2(-2.5, -1.5) * radius1);
	...
	float2 radius2 = 0.1 * GetInvResolution();
	...
	SetOutput((c_center * 0.0 + bloom_sum) * dist);
}